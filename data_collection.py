...
def init_csv_file(badge_name, db_name):
    """Initialize CSV file with headers"""
    global csv_filename
    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    csv_filename = f"{badge_name}_data_{timestamp}.csv"
...
def save_to_csv(timestamp, badge_name, sound, rssi, acceleration, raw_data):
    """Save a single data point to CSV file"""
    global csv_filename
    if csv_filename:
        try:
...
async def connection_run(address):
    ...
    received_data = []
    stop_collection = False
    # Initialize CSV file for this badge
    csv_file = init_csv_file(BadgeName, DBName)
...
async def main():
    ...
    global detected_badge_addresses,Total_detected_device,DBName,client_badge_mapping
